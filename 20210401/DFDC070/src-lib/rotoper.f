...
C---- Absolute frame velocities
        ...
        VABS(1,IR,NR) = VIND(1,IR,NR) + QINF    + WWA
...
C
      ...
      WRITE(*,100) NAME
      RPM = 30.0*OMEGA(NR)/PI
      WRITE(*,25)  QINF,QREF,OMEGA,RPM
C
      WRITE(*,200) NBEL
      DO N = 1, NBEL
        WRITE(*,300) XBLE(N),YBLE(N),
     &               XBTE(N),YBTE(N),
     &               XBREFE(N), YBREFE(N),
     &               VOLUMV(N), ASURFV(N),
     &               XBCENV(N), YBCENV(N),
...
C     
 ...
 25   FORMAT(' QINF  =',F12.4,
     ...
     &      /' OMEGA =',F12.4,
     &      /' RPM   =',F12.4)
  200 FORMAT(/'Current duct with ', I4, ' elements:')
...
C
       IF(IRTYPE(N).EQ.1) THEN
        WRITE(*,100) NAME
        RPM = 30.0*OMEGA(N)/PI
        WRITE(*,110) QINF,QREF,OMEGA(N),RPM
C
        WRITE(*,120) N,RHUB(N),RTIP(N),ADISK(N)
        WRITE(*,200)
C 
        DO IR = 1, NRC
          WRITE(*,210) YRC(IR,N),YRC(IR,N)/RTIP(N),
     &                 BGAM(IR,N)
        END DO
       ENDIF
C
...
C
        ...
        WRITE(*,110) QINF,QREF,OMEGA(N),RPM
C
        WRITE(*,120) N,NRBLD(N),RHUB(N),RTIP(N),ADISK(N)
        BLDS = FLOAT(NRBLD(N))
        WRITE(*,200)
C
        DO IR = 1, NRC
...
C     
  ...
  110 FORMAT(' QINF  =',F12.4,
     ...
     &      /' RPM   =',F12.4)
C
  120 FORMAT(/'Current rotor for disk # ',I3,
     &       /'  # blades ',I4,
     &       /'    Rhub   ',F12.5,
     &       /'    Rtip   ',F12.5,
     &       /'    Aswept ',F12.6)
...
C
        DO ID = 1, NDDEF(N)
          WRITE(*,210) ID,XDDEF(ID,N),YDDEF(ID,N),CDADEF(ID,N)
        END DO
C
      END DO
C     
  ...
  110 FORMAT(' QINF  =',F12.4,
     ...
     &      /' RPM   =',F12.4)
C
  120 FORMAT(/'# Drag area objects defined: ', I4)
  130 FORMAT(/'Drag area: ', I4)
  200 FORMAT('   i        x          r       CDave')
c             a1234567890112345678901123456789011234567890112345678901    
  210 FORMAT(I4,2X,7G11.4)
C
      RETURN
      END ! SHOWDRAGOBJ
...
C---- change type for disk so blades are defined
      ...
      LBLDEF = .TRUE.
C     
 1400 FORMAT(/'Blades for disk ',I3,' designed with ',I4,' blades:'
     &       /'     r/R         r        Ch  Beta0deg',
c              12345678912345678901234567890123456789
     &        '      CL        W     BGam ',
c              123456789123456789123456789
     &        '   dCLdA  alfadeg')
c              123456789123456789
 1410 FORMAT(F9.4,2F10.4,6F9.3)
C
      RETURN
      END ! DESBLADE
...
C---- Skip forces for tip gap (only on rotor)
        IF(I.EQ.NRC .AND. TGAP.GT.0.0 .AND. OMEGA(N).NE.0.0) GO TO 1000
C
        XI = YRC(I,N)/RTIP(N)
...
C
 ...
 1003 FORMAT(1X,78('-'))
 1004 FORMAT(50X)
 1005 FORMAT(' (External slipstream present)',19X)
C
...
C---- Dump overall case data
      WRITE(LU,1003) 
      WRITE(LU,1010) 
      WRITE(LU,1012) QINF,  ALTH/1000.0, DELTAT,
     &               RHO,   VSO,   RMU,
     &               TDIM,  PDIM,  EFFTOT,
     &               TVDIM, PVDIM, EFFIND,
     &               TDUCT, QDIM,  EIDEAL
...
C
 1010 FORMAT(' Flow condition and total forces')
 1012 FORMAT(/'  Vinf(m/s) :',F10.3,4X,'Alt.(km)   :',F9.3, 3X,
     &   'DeltaT(dgC):',F9.4,
...
C---- NOTE: should only dump blade data to NRC-1 for tip gap case
       DO I=1, NRC, IADD
         ...
         SCHAR = ' '
         IF(LSTALLR(I,N)) SCHAR = 's'
         WRITE(LU,1130)  I,XI,CHI,BDEG,CLR(I,N),SCHAR,CDR(I,N),
...
C
C---- Rotor data
 1120 FORMAT('   i    r/R     c/R  beta(deg)      CL       Cd',
     &       '   RE      ',   '    Mach      B*Gam')
 1122 FORMAT('   i    r/R     c/R    beta(deg)    CL       Cd',
     &       '   REx10^',I1,  '    Mach      B*Gam')
 1130 FORMAT(2X,I2,F7.3,F8.4,F8.2,2X,F8.3,1X,A1,F8.4,1X,
     &       F7.2,4X,F7.3,1X,F10.3)
C
C---- Actuator disk data
 1220 FORMAT('   i    r/R     c/R  beta(deg)      CL       Cd',
     &       '     RE    ',   '    Mach      B*Gam')
 1230 FORMAT(2X,I2,F7.3,8X,  8X,  3X,8X,  1X,1X,8X,  1X,
     &       7X,  4X,F7.3,1X,F10.3)
C
C
