...
C
      ...
      DCDCL2S =  0.020   ! d(Cd)/d(Cl**2) secondary (annulus) drag
      ...
      TOC   =  0.1       ! section thickness ratio
C
...
C
C---- Interpolate design CL to new radial stations
      IF(NRCSAV.GT.0) THEN
       DO I = 1, NRCSAV
         T4(I) = CLDES0(I)
       END DO
       ...
       DO I = 1, NRC
         CLDES(I) = SEVAL(YRC(I,N),T4,T4S,YRC0,NRCSAV)
       END DO
      ENDIF
C
      RETURN
...
cc          READ(LINE,*,ERR=210) REREF,REXP
     ...
     &                 CMCON,MCRIT,TOC,REREF,REXP)
        ...
        IF(INDEX(LINE,'ENDAERO').NE.0) THEN
          WRITE(*,*) 'AERO data read for disk # ',NR,
     &               ' # sections ',NAERO(NR)
        ELSE
...
C
 13     CALL RDLINE(LU,LINE,ICNT)
        IF(INDEX(LINE,'ENDROTOR').NE.0) THEN
          ...
          WRITE(*,*) ' XDisk         = ',XDISK(NR)
          WRITE(*,*) ' # blades      = ',NRBLD(NR)
          WRITE(*,*) ' #Rotor points = ',NRSTA
          IF(NRDEF(NR).GE.2) THEN
           WRITE(*,*) 'Rotor blade defined with #points = ',NRDEF(NR)
           WRITE(*,*) '   R           CH           Beta'
           DO IR = 1, NRDEF(NR)
...
C
 18     CALL RDLINE(LU,LINE,ICNT)
        IF(INDEX(LINE,'ENDACTDISK').NE.0) THEN
          ...
          WRITE(*,*) ' #Rotor points = ',NRSTA
          IF(NRDEF(NR).GE.2) THEN
           WRITE(*,*) 'Actuator disk defined with #points = ',NRDEF(NR)
           WRITE(*,*) '   R           BGAM'
           DO IR = 1, NRDEF(NR)
...
C
 19      CALL RDLINE(LU,LINE,ICNT)
         IF(INDEX(LINE,'ENDDRAGOBJ').NE.0) THEN
           NDOBJ = ND1
           WRITE(*,*) 'DRAGOBJ ', ND1, ' read from file'
           WRITE(*,*) ' #Drag object points = ',NDDEF(ND1)
           IF(NDDEF(ND1).GE.2) THEN
            WRITE(*,*) 'Drag object defined with #points = ',NDDEF(ND1)
            WRITE(*,*) '   X            R           CDA'
            DO ID = 1, NDDEF(ND1)
...
C---- PANE data (respacing data for surface points
     ...
     &                           CRRAT(IRPN,IEL)
        ...
        IF(INDEX(LINE,'ENDPANE').NE.0) THEN
          WRITE(*,*) 'PANELING for ', NBIN, ' elements read from file'
          LRSPCDEF = .TRUE.
          LRSPCUSR = .TRUE.
        ENDIF
C
...
C
        IF(ERROR) THEN
         WRITE(*,*) 'No repaneling parameters from file ',PFILE(1:K+4)
        ELSE
         WRITE(*,*) 'Repaneling parameters from file ',PFILE(1:K+4)
        ENDIF
      ENDIF
C
C=======================================================================
...
C...................................................................
 1000 FORMAT(A)
 1100 FORMAT('DFDC Version ',F5.2,' '/A)
C
 1102 FORMAT(/'OPER',
     &       /'!        Vinf         Vref          RPM')
