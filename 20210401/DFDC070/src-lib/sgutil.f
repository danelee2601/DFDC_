...
C
      WRITE(*,3020) NG, FSL, FSR, CVEX, SMOF
      DO IREF = 1, NREF
        IF(CRRAT(IREF) .NE. 0.0  .AND.  SREF1(IREF) .LT. SREF2(IREF))
     &      WRITE(*,3040) IREF ,SREF1(IREF),SREF2(IREF),CRRAT(IREF)
      ENDDO
 3020 FORMAT(
     &  /'  ........................................................'
     & //'  Number of panel nodes =', I5
     &  /'  Prescribed panel node spacing parameters ...'
     &  /'     dsL/dsAvg =',F7.3, '      dsR/dsAvg =',F7.3,
     &  /'    curvature scaling exponent =',F8.4,
     &  /'    curvature smoothing factor =',F8.4 )
 3040 FORMAT(
     &   '    station',I2,' refinement between s/smax =',2F8.4,
     &   ' ,    ds/dsAvg =',F8.3)
C
      WRITE(*,3100)
      WRITE(*,3110) DLE/DSAVG,   DTE/DSAVG,
     &            DSMIN/DSAVG, DSMAX/DSAVG
C
 3100 FORMAT(/'  Actual resulting surface spacings ...')
 3110 FORMAT( '     dsL/dsAvg =',F8.4,'     dsR/dsAvg =',F8.4,
     &       /'   dsmin/dsAvg =',F8.4,'   dsmax/dsAvg =',F8.4 )
C
      IF(LQUERY) THEN
